...
#write.table(dataSub[c("bblid","scanid")],"~/voxelPaper/data/fulllist_bblid_scanid_.csv", row.names=F, col.names=F, sep=",")

dataPaths <- read.csv("/home/agarza/voxelPaper/data/fulllist_paths_.csv", header=F,as.is=T)
names(dataPaths) <- c("bblid","scanid","asl.path")

dataSub <- merge(dataSub, dataPaths, by=c("bblid","scanid"))

saveRDS(dataSub, "~/voxelPaper/data/voxelPaper_FullSubjDataset.rds")
